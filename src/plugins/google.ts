...
export const googleOAuth = OAuth2Plugin({
  ...
  getUserInfo: async (accessToken: string, req: PayloadRequest) => {
    ...
    return { name: user.name, email: user.email, sub: user.sub };
  },
  /**
  ...
  getToken: async (code: string, req: PayloadRequest) => {
    const redirectUri = `${NEXT_PUBLIC_SERVER_URL}/api/users/oauth/google/callback`;
    ...
    // Ensure that user with same email exists in database
    const response = await fetch("https://www.googleapis.com/oauth2/v3/userinfo", {
      headers: { Authorization: `Bearer ${token}` },
    });
    const user = await response.json();
    const dbUsers = await req.payload.find({
      collection: "users",
      where: { email: { equals: user.email } },
    });
    if (dbUsers.docs.length == 0) {
      // TODO: show this to user somehow
      throw new APIError("User not found", 404, undefined, true);
    }
